  ...
  if not os.path.isfile(fname):
    ...
    for i,ii in enumerate(info['temp_r']):
      for j,k in zip(info['Power_Factor_mu'][i],info['Kappa_mu'][i]):
         temp_zt.append(.001*j*ii/(k+k_latt))
      zt.append(temp_zt)
      temp_zt=[]
    zt=np.array(zt)
    info['zt_mu']=zt
...
def run_dir(dir='/users/knc6/Software/JARVIS-TOOLS/jarvis/jarvis/vasp/examples/SiOptb88'):
 for i in os.listdir(dir):
  if 'json' in i:
    if 'MAIN-RELAX' in i:
      try:
        mvrun=str(dir)+str('/')+str(i.split('.json')[0])+str('/vasprun.xml')
    ...
    if 'MAIN-MBJ' in i:
      try:
        mbjvrun=str(dir)+str('/')+str(i.split('.json')[0])+str('/vasprun.xml')
    ...
    if 'MAIN-OPTICS' in i:
      try:
        mopvrun=str(dir)+str('/')+str(i.split('.json')[0])+str('/vasprun.xml')
...
def plot_prop_mu(prop='Seebeck_mu',bfile='',temp=300,filename='seebplot.png'):
    ...
    props = ["Conductivity_mu", "Seebeck_mu", "Kappa_mu", "Effective_mass_mu" ,"Power_Factor_mu", "Carrier_conc_mu", "Hall_carrier_conc_trace_mu","zt_mu"]
    props_lbl = ["Conductivity", "Seebeck", "$K_{el}$", "Effective mass","Power Factor", "Carrier concentration", "Hall carrier conc.","ZT"]
