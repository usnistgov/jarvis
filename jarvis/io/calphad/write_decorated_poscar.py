...
def get_selective_dyn_decorated_atoms(atoms):
 ...
 for i,j in zip(wsymbols,frac_coords):
         ...
         #print ()
         #print ()
 decorated_atoms = Atoms(lattice_mat = atoms.lattice_mat, elements = atoms.elements, coords = frac_coords, cartesian = False, props = props)
 return decorated_atoms,hall_number,wsymbols

if __name__ == "__main__":
 atoms = Poscar.from_file('/rk2/knc6/ZenGen/VASP-CALCS/C14EndMembersCalculationSetup/AlAlAl/CONTCAR').atoms
 atoms = Poscar.from_file('/rk2/knc6/ZenGen/VASP-CALCS/C14EndMembersCalculationSetup/TiTiFe/CONTCAR').atoms
 decorated_atoms,hall_number, wsymbols = get_selective_dyn_decorated_atoms(atoms)
 print (decorated_atoms, wsymbols, hall_number)
 #import sys
 #sys.exit()
 x = []
 for i in glob.glob('/rk2/knc6/ZenGen/VASP-CALCS/C14EndMembersCalculationSetup/*/CONTCAR'):
     print (i)
 ...
 for i in glob.glob('/rk2/knc6/ZenGen/VASP-CALCS/Ti-Al-FeTau2NewRound_11-13-2019/*/CONTCAR'):
     print (i)
     atoms = Poscar.from_file(i).atoms
     decorated_atoms,hall_number,wsymbols = get_selective_dyn_decorated_atoms(atoms)
     print (decorated_atoms,hall_number,wsymbols)
