...
class WriteWin(object):
    def __init__(self, struct="",dis_num_iter=100, kmesh_tol=0.000000001,dis_mix_ratio =0.5,  num_iter =100, num_print_cycles =50, frozen_tol=2, efermi=None, name="win.input", soc=True, semi_core_states=None):
        self.struct = struct
        self.efermi = efermi
        self.name = name
        self.soc = soc
        ...
        if self.semi_core_states is None:
            f=open('default_semicore.json','r')
...
if __name__ == "__main__":
    ...
    WriteWin(struct=Si,efermi=0.0).write_win()
