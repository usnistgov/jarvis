...
def mismatch_strts(film=[],subs=[]):
   ...
   try:
    ...
    angle1=np.arccos(np.dot(a1, a2) / np.linalg.norm(a1) / np.linalg.norm(a2)) * 180 / np.pi
    angle2=np.arccos(np.dot(b1, b2) / np.linalg.norm(b1) / np.linalg.norm(b2)) * 180 / np.pi
    ...
    mat2d_fake_c = film.lattice.matrix[2, :] / np.linalg.norm(
        film.lattice.matrix[2, :]) * 5.0
    ...
    mat2d_latt_fake = Lattice(np.array(
        [
            film.lattice.matrix[0, :],
            film.lattice.matrix[1, :],
            mat2d_fake_c
        ]))
...
def get_direct_hetero(bulk_film='',bulk_subs='',film_miller=(1,1,1),film_subs=(1,1,1),seperation=3.0,vacuum=10.0,film_layers=3,bulk_layers=3):
    ...
    slab_film=get_ase_surf(strt=bulk_film_cvn,miller=film_miller,layers=film_layers,vacuum=vacuum)
    slab_subs=get_ase_surf(strt=bulk_subs_cvn,miller=film_subs,layers=bulk_layers,vacuum=vacuum)
