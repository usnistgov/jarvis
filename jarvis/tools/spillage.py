...
def overlap_so_spinpol(wf_noso, wf_so):
    ...
    for nk1  in range(1,noso._nkpts+1):#no spin orbit kpoints loop
        ...
        for nk2  in range(1,so._nkpts+1): #spin orbit
            kso = so._kvecs[nk2-1,:]
            if isclose(kso[0], knoso[0]) and isclose(kso[1], knoso[1]) and isclose(kso[2], knoso[2]): #do kpoints match?
        ...
        for nk2  in range(1,so._nkpts+1): #spin orbit
            ...
            if isclose(kso[0], knoso[0]) and isclose(kso[1], knoso[1]) and isclose(kso[2], knoso[2]): #do kpoints match?
                ...
                for n1 in range(1,nelec_up+1):
                    Vnoso[0:vs//2,n1-1] = noso.readBandCoeff(ispin=1, ikpt=nk1, iband=n1, norm=False)[0:vs//2]
                for n1 in range(1,nelec_dn+1):
                    Vnoso[vs//2:vs,n1-1 + nelec_up] = noso.readBandCoeff(ispin=2, ikpt=nk1, iband=n1, norm=False)[0:vs//2]
                ...
                if gamma_k[-1] > 0.5:
                    print ('nk1 nk2 kpoint gamma_k ', nk1, nk2, kso, knoso, np.real(gamma_k[-1]), '!!!!!!!!!!')
    ...
    print ('no spin-orbit gaps', '{:+.3f}'.format(float(noso_lumo-noso_homo)), '{:+.3f}'.format(noso_direct), '   ', [noso_homo,noso_lumo])
    print ('spin-orbit gaps   ', '{:+.3f}'.format(float(so_lumo-so_homo)), '{:+.3f}'.format(so_direct), '   ',[so_homo,so_lumo])
    print ()
    
    print ('gamma max', np.real(gmax), ' at k =  ', kmax)

    return np.real(gmax), np.real(gamma_k), kmax, kpoints,noso_direct,so_direct,x,y,so_lumo,so_homo,noso_lumo,noso_homo
...
if __name__ == '__main__':
    ...
    gamma_max, gamma_k, kmax, kpoints,noso_direct,so_direct,x,y,so_lumo,so_homo,noso_lumo,noso_homo = overlap_so_spinpol(wf_noso, wf_so)
    print (gamma_max)
