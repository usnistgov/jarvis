...
s1 = Poscar.from_file(os.path.join('.','POSCAR-JVASP-652'))
s2 = Poscar.from_file(os.path.join('.','POSCAR-JVASP-664'))


def test_zur():
    info = mismatch_strts(film=s1.atoms, subs=s2.atoms)
    #print (round(info['mismatch_u'],3), round(info['mismatch_angle'],3))
    assert (round(info['mismatch_u'],3), round(info['mismatch_angle'],3)) == (0.002, 0.0)
#test_zur()
