...
class SolarEfficiency(object):
        ...
        def calculate_SQ(self,bandgap_ev, temperature=300, fr=1, plot_current_voltage=False):
            """
            Args:
...
if __name__ == "__main__":
   ...
   v=Vasprun('/rk2/knc6/JARVIS-DFT/Elements-bulkk/mp-149_bulk_PBEBO/MAIN-MBJ-bulk@mp-149/vasprun.xml')
   dirgap=v.get_dir_gap
   indirgap=v.get_indir_gap
   en,abz=v.avg_absorption_coefficient 
   abz=abz*100
   eff = SolarEfficiency().slme(en, abz, indirgap, indirgap, plot_current_voltage=False)
   print ('SLME',100*eff)
