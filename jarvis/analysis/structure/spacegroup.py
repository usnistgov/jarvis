...
class Spacegroup3D(object):
    ...
    def refined_atoms(self):
         phonopy_atoms = (self._atoms.lattice_mat, self._atoms.frac_coords, self._atoms.atomic_numbers)
         lattice, scaled_positions, numbers = spglib.refine_cell(phonopy_atoms, self._symprec, self._angle_tolerance)
         elements = self._atoms.elements
         el_dict = {}
         for i in elements:
           el_dict.setdefault(Specie(i).Z,i)     
         ref_elements =  [el_dict[i] for i in numbers]
         ref_atoms=Atoms(lattice_mat=lattice,elements=ref_elements,coords=scaled_positions,cartesian=False)
         return ref_atoms
    ...
    def conventional_standard_structure(
        ...
        new_struct = Atoms(lattice_mat=latt.matrix, elements=struct.elements, coords=new_coords, cartesian=False)
        return new_struct
